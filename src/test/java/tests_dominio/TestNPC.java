...
public class TestNPC {
	...
	// el nombre de este método confunde, ya que en realidad aumenta el
	// ataque...
	npc.setAtaque(100);
	...
	/*
	 * No estoy seguro de que se supone que hace el atributo magia. Asumo
	 * que se supone que debería afectar el daño mágico? De un modo u otro
	...
	NonPlayableCharacter npc = new NpcVampiro("Arturo", 1);
	Personaje per = new Orco("Janson", new Guerrero(), 0);
	per.setInvulnerabilidad(true);
	Assert.assertEquals(0, npc.atacar(per));
	Assert.assertEquals(0, npc.daniarSalud(per, 1000));
	Assert.assertEquals(0, npc.daniarEnergia(per, 1000));
    }
