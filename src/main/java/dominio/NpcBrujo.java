...
public class NpcBrujo extends NonPlayableCharacter {
	...
	public int serAtacado(int daño) {
		if (this.getRandom().nextDouble() <= CHANCE_EVADIR) {
			...
			// aplico una reducción al daño recibido
			if (this.turnosCampoEnergia > 0) {
				daño = (int) (daño * CAMPO_ENERGIA_PORCENTAJE_BLOQUEADO);
			}
			if (daño > 0) {
				this.salud -= daño;
			}
	...
	public void jugarTurno(Peleable objetivo) {
		...
		if (turnosEnvenenar > 0) {
			int daño = ENVENENAR_DANIO_BASE + ENVENENAR_MULT_DANIO * this.getNivel();
			daniarSalud(objetivo, daño);
	 ...
	 */
	private void efectuarAlgunAtaque(Peleable atacado) {
	 ...
	 */
	public boolean bolaDeFuego(Peleable atacado) {
		if (this.energia <= BOLA_FUEGO_COSTE_ENERGIA) {
			return false;
		}

		int daño = (int) (this.getAtaque() * BOLA_FUEGO_MULT_DANIO);

		if (this.getRandom().nextDouble() <= BOLA_FUEGO_CHANCE_CRITICO) {
			daño = (int) (daño * BOLA_FUEGO_MULT_CRITICO);
		}

		daniarSalud(atacado, (int) this.getRandom().aplicarDispersión(daño, BOLA_FUEGO_DESVIO));
	...
	/**
	 * Ataque que causa daño al rival a lo largo de una determinada cantidad de
	 * turnos.
	 * 
	 * @param atacado
	 *            Objetivo a atacar
	 * @return Devuelve true si se pudo efectuar el ataque
	 */
	public boolean envenenar(Peleable atacado) {
		if (this.energia <= ENVENENAR_COSTE_ENERGIA) {
			return false;
		}

		int daño = ENVENENAR_DANIO_BASE + ENVENENAR_MULT_DANIO * this.getNivel();
		daniarSalud(atacado, daño);
		this.energia -= ENVENENAR_COSTE_ENERGIA;
		// el -1 es porque el primer daño lo aplica en el momento
	...
	/**
	 * Habilidad que permite al brujo reducir el daño recibido en un porcentaje.
	 * 
	 * @return Devuelve true si se pudo utilizar la habilidad.
	 */
	public boolean campoDeEnergia() {
		...
		if ((double) (this.getEnergia() / this.getEnergiaTope()) < CAMPO_ENERGIA_PORCENTAJE_NECESARIO) {
			return false;
		}
