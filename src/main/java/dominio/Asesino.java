 ...
 * dos constructores. El otro constructor, llama al de la clase padre (Casta)
 * pasándole los argumentos recibidos por el constructor hijo y luego inicializa
...
public class Asesino extends Casta {
     ...
     * @param probCrit Probabilidad de que el personaje realice un golpe crítico
     * @param evasion Probabilidad de que el personaje evite un golpe crítico
     * @param danioCrit Valor por el cual será multiplicado el golpe básico
     */
     ...
     * el atacante(caster) posea 10 o más del atributo energía ya que estos se
     * descuentan luego de comprobar que los posee, de lo contrario el ataque no
     * será posible y se retornará false De ser posible el ataque, se llama al
     * método serAtacado() del argumento atacado el cual actuará de cierta
     * manera dependiendo de que clase sea el argumento atacado
     * @param caster Personaje atacante
     * @param atacado puede recibir como argumento una instancia de Personaje o
     *            de NPC (NonPlayableCharacter) ya que ambas implementan la
     *            interface Peleable
	...
	if (caster.getEnergia() >= ENERGIAMINIMA) {
	    ...
	    return (atacado.serAtacado((int) (caster.getAtaque() * caster.getCasta().getDanioCritico())) > 0);
	}
     ...
     * @param caster Personaje que realiza el ataque, en este caso no es un
     *            ataque si no un aumento en las caracteristicas del caster.
